...
function drawThreeGeo(json, radius, shape, options) {
	...
	var webGLGroup = new THREE.Group();
    ...
    for (var geom_num = 0; geom_num < json_geom.length; geom_num++) {
    	if (json.features[geom_num].properties['color']!=null) {
    		console.log(json.features[geom_num].properties['color']);
    		options.color = json.features[geom_num].properties['color'];
    	}
    	
        if (json_geom[geom_num].type == 'Point') {
            convertCoordinates(json_geom[geom_num].coordinates, radius);            
            webGLGroup.add(drawParticle(y_values[0], z_values[0], x_values[0], options));
            ...
            webGLGroup.add(drawLine(y_values, z_values, x_values, options));
            ...
            for (var segment_num = 0; segment_num < json_geom[geom_num].coordinates.length; segment_num++) {
                ...
                webGLGroup.add(drawPolygon(y_values, z_values, x_values, options));
            }                            
...
function drawPolygon(x_values, y_values, z_values, options) {
	...
	polyShape.moveTo(x_values[0], y_values[0]);
	for (var i0 = 0; i0 < x_values.length; i0++) {
		polyShape.lineTo(x_values[i0], y_values[i0]);
    }
	var poly_geom = new THREE.ShapeGeometry( polyShape );
    //var poly_geom = new THREE.Geometry();
    //createVertexForEachPoint(poly_geom, x_values, y_values, z_values);
	
    var poly_material = new THREE.MeshLambertMaterial(options);
    var polygon = new THREE.Mesh(poly_geom, poly_material);
